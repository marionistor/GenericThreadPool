...
void enqueue_task(os_threadpool_t *tp, os_task_t *t)
{
	...
	tp->first_enqueue = 1;
	pthread_cond_signal(&tp->empty_queue);
...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	while (queue_is_empty(tp)) {
		if (tp->num_waiting_threads == tp->num_threads && tp->first_enqueue) {
			tp->num_waiting_threads--;
			tp->num_threads--;
		...
		pthread_cond_wait(&tp->empty_queue, &tp->queue_mutex);
	}
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	rc = pthread_mutex_init(&tp->queue_mutex, NULL);
	DIE(rc != 0, "pthread_mutex_init");

	rc = pthread_cond_init(&tp->empty_queue, NULL);
